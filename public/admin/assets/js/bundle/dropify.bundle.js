...
!function (e, i) {
    ...
    function i(i, t) {
        if (window.File && window.FileReader && window.FileList && window.Blob) {
            var dropifyMessages = window.dropifyMessages || {};
            var s = {
                defaultFile: "",
                maxFileSize: 0,
                minWidth: 0,
                ...
                messages: {
                    "default": dropifyMessages.default || "Drag and drop a file here or click",
                    replace: dropifyMessages.replace || "Drag and drop or click to replace",
                    remove: dropifyMessages.remove || "Remove",
                    error:  dropifyMessages.error || "Ooops, something wrong appended."
                },
                error: {
    ...
    return i.prototype.onChange = function () {
        ...
        // add new input to check if the image removed
        let newInput = $('<input>', {
            type: 'hidden',
            name : this.input.attr("name") + '_value',
            id : this.input.attr("name") + '_value',
            value : this.settings.defaultFile || "",
        });
        ...
        // add new input to check if the image removed
        let image_value = this.input.attr("name")
        if (typeof image_value !== 'undefined') {
            if($('#'+image_value + '_value') != null){
                $('#'+image_value + '_value').val(this.file.name);
            }
        }
        // end
        ...
        // clear image value if image removed
        let image_value = this.input.attr("name")
        if($('#'+image_value + '_value') != null){
            $('#'+image_value + '_value').val('');
        }
        // end
