...
;(function ($, window, document, undefined) {
    ...
    multiField.prototype = {
        ...
        cloneSection: function () {
            ...
            // Clear input values
            $('input[type!="hidden"],input[type!="radio"],textarea', newChild).each(function () {
                $(this).val('');
            });
            ...
            // Fix class input.index-plus : update name [i] to [i+1]
            newChild.find('input.index-plus').each(function () {
                var name = $(this).attr('name');
                $(this).attr('name', name.replace(/([0-9]+)/g, 1 * (name.match(/([0-9]+)/g)) + 1));
            });

            newChild.find('select.index-plus').each(function () {
                var name = $(this).attr('name');
                $(this).attr('name', name.replace(/([0-9]+)/g, 1 * (name.match(/([0-9]+)/g)) + 1));
            });
            newChild.find('textarea.index-plus').each(function () {
                var name = $(this).attr('name');
                $(this).attr('name', name.replace(/([0-9]+)/g, 1 * (name.match(/([0-9]+)/g)) + 1));
            });
            // Fix clear dropify
          //  newChild.find('.dropify-clear').click();
